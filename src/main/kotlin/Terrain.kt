...
class Terrain(
    ...
    val heightMap = Array(width) { DoubleArray(height) }
    ...
    fun getHeight(x: Int, y: Int): Double = when {
        x !in 0..<width || y < 0 || y >= height -> 0.0
        else -> heightMap[x][y]
    }

    fun getColorForHeight(height: Double): Color = when {
